/**
 * Switch Language Tool Tests
 * 
 * Tests for the switchLanguage server-side tool.
 */

import { describe, test, expect, beforeEach } from 'bun:test';
import { executeSwitchLanguageTool } from '../src/lib/server-tools/switch-language';
...
describe('switchLanguage tool', () => {
  describe('basic functionality', () => {
    test('should switch language successfully', async () => {
      const context = createTestContext();
      
      const result = await executeSwitchLanguageTool(
        { languageCode: 'es' },
        context
      );
  ...
  describe('Last Voice Per Language (Runtime Tracking)', () => {
    ...
    test('should track different voices for different languages', async () => {
      ...
      // Switch to Spanish
      const result1 = await executeSwitchLanguageTool(
        { languageCode: 'es' },
        context
      );
      ...
      // Switch to French
      const result2 = await executeSwitchLanguageTool(
        { languageCode: 'fr' },
        context
      );
  ...
  describe('Voice Selection Priority Order', () => {
    test('should follow priority: session memory > token config > gender', async () => {
      ...
      const result3 = await executeSwitchLanguageTool(
        { languageCode: 'es' },
        context3
      );
