...
Projects that consume the engine and/or `@vowel.to/client`:

### Fieldr

| Property | Value |
|----------|-------|
| **Path** | `/home/k/Git/vowel/fieldr` |
| **Client** | `@vowel.to/client` linked via `bun link @vowel.to/client` |

**Setup after building client:**
```bash
cd /home/k/Git/vowel/workspace/client && bun run build && bun link
cd /home/k/Git/vowel/fieldr && bun link @vowel.to/client
```

**Tool registration:** `src/vowel.client.ts` (around line 371) — `patchReportBuilderFields` tool.
...
**If the user explicitly asks for GitNexus:** read `.claude/skills/gitnexus/gitnexus-guide/SKILL.md` first, then other skills there as needed. Run `npx gitnexus analyze` if tools report a stale index. For `gitnexus://repo/...` URIs, this tree is usually registered as **engine**.
