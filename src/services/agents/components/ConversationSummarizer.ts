 ...
 * a fast LLM (default: openai/gpt-oss-20b).
 * 
 ...
 * 
 * This component was restored from the previous implementation (usage-optimization branch)
 * where it successfully handled very long conversations (hours of voice chat).
 * 
...
export class ConversationSummarizer {
  ...
  constructor(config: SummarizationConfig) {
    ...
    getEventSystem().info(EventCategory.LLM, '📚 [ConversationSummarizer] Initialized');
    getEventSystem().info(EventCategory.LLM, `   Enabled: ${this.config.enabled}`);
    getEventSystem().info(EventCategory.LLM, `   Active Window: ${this.config.activeWindowSize} messages`);
    getEventSystem().info(EventCategory.LLM, `   Summarization Buffer: ${this.config.summarizationBufferSize} messages`);
    getEventSystem().info(EventCategory.LLM, `   Max Summaries: ${this.config.maxSummaries}`);
    getEventSystem().info(EventCategory.PROVIDER, `   Provider: ${this.config.summarizationProvider}`);
    getEventSystem().info(EventCategory.LLM, `   Model: ${this.config.summarizationModel}`);
  }
  ...
  public addMessage(message: CoreMessage): void {
    ...
    if (this.activeWindow.length > this.config.activeWindowSize) {
      ...
      getEventSystem().info(EventCategory.LLM, `📤 [ConversationSummarizer] Moved message to summarization buffer`);
      getEventSystem().info(EventCategory.LLM, `   Active Window: ${this.activeWindow.length}/${this.config.activeWindowSize}`);
      getEventSystem().info(EventCategory.LLM, `   Summarization Buffer: ${this.summarizationBuffer.length}/${this.config.summarizationBufferSize}`);

      // Check if summarization buffer is full
      if (this.summarizationBuffer.length >= this.config.summarizationBufferSize) {
        getEventSystem().info(EventCategory.LLM, `🔄 [ConversationSummarizer] Summarization buffer full, triggering summarization...`);
  ...
  private triggerSummarization(): void {
    if (this.isProcessing) {
      getEventSystem().warn(EventCategory.LLM, '⚠️ [ConversationSummarizer] Summarization already in progress, skipping...');
      return;
    }
    ...
    getEventSystem().info(EventCategory.LLM, `🤖 [ConversationSummarizer] Starting async summarization of ${messagesToSummarize.length} messages...`);
    ...
    this.summarizeMessages(messagesToSummarize)
      .then((summary) => {
        ...
        if (this.summaryBuffer.length > this.config.maxSummaries) {
          ...
          getEventSystem().info(EventCategory.LLM, `🗑️ [ConversationSummarizer] Removed oldest summary (${removed.messageCount} messages)`);
        }

        getEventSystem().info(EventCategory.LLM, `✅ [ConversationSummarizer] Summarization complete`);
        getEventSystem().info(EventCategory.LLM, `   Summary Buffer: ${this.summaryBuffer.length}/${this.config.maxSummaries}`);
        getEventSystem().info(EventCategory.LLM, `   Total Summarized: ${this.totalMessagesSummarized} messages`);

        this.isProcessing = false;
      })
      .catch((error) => {
        getEventSystem().error(EventCategory.LLM, '❌ [ConversationSummarizer] Summarization failed:', error);
        
        // On error, put messages back in buffer (at the front)
        this.summarizationBuffer.unshift(...messagesToSummarize);
        getEventSystem().info(EventCategory.LLM, `🔄 [ConversationSummarizer] Restored ${messagesToSummarize.length} messages to buffer`);
  ...
  private async summarizeMessages(messages: CoreMessage[]): Promise<string> {
    ...
    try {
      ...
      const provider = getProvider(this.config.summarizationProvider as any, {
      ...
      // Generate summary with low reasoning effort
...
Summary:`,
      ...
      getEventSystem().error(EventCategory.LLM, '❌ [ConversationSummarizer] Summarization error:', error);
      throw error;
    }
  }
  ...
  public clear(): void {
    ...
    getEventSystem().info(EventCategory.LLM, '🧹 [ConversationSummarizer] Cleared all buffers');
  }
