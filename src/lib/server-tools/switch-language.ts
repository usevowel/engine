/**
 * Switch Language Tool Executor
 * 
 * Server-side executor for the 'switchLanguage' tool.
 * Allows the AI to switch the language dynamically during a session.
 * 
 * Also automatically selects an appropriate TTS voice for the new language,
...
/**
 * Execute the switchLanguage tool
 * 
 * Switches the current language for TTS and STT.
 ...
 */
export async function executeSwitchLanguageTool(
  ...
  if (!languageCode || typeof languageCode !== 'string') {
    getEventSystem().error(EventCategory.SESSION, `❌ [SwitchLanguage] Invalid language code: ${languageCode}`);
    return {
      success: false,
    ...
    if (LANGUAGE_ALIASES[normalizedCode]) {
      ...
      if (foundCode) {
        ...
        getEventSystem().error(EventCategory.SESSION, 
          `❌ [SwitchLanguage] Unknown language: ${languageCode}`);
        return {
          success: false,
  ...
  if (!WHISPER_LANGUAGES[normalizedCode]) {
    ...
    getEventSystem().error(EventCategory.SESSION, 
      `❌ [SwitchLanguage] Unsupported language code: ${normalizedCode}`);
    return {
      success: false,
  ...
  getEventSystem().info(EventCategory.SESSION, 
    `🌍 [SwitchLanguage] Tool called to switch language: ${normalizedCode} (${languageName}, original: ${languageCode})`);
  ...
  if (sessionData.languageDetectionService) {
    ...
    getEventSystem().info(EventCategory.SESSION, 
      `✅ [SwitchLanguage] Updated language detection service: ${normalizedCode}`);
  }
  ...
  getEventSystem().info(EventCategory.SESSION, 
    `📝 [SwitchLanguage] Tracked ${newVoice} as last voice for ${normalizedCode}`);
  ...
  if (sessionData.config) {
    ...
    if (newVoice !== previousVoice) {
      getEventSystem().info(EventCategory.SESSION, 
        `🎤 [SwitchLanguage] Voice changed: ${previousVoice || 'none'} → ${newVoice}`);
    } else {
      getEventSystem().info(EventCategory.SESSION, 
        `🎤 [SwitchLanguage] Voice unchanged: ${newVoice} (already appropriate for ${normalizedCode})`);
    }
  }
  
  getEventSystem().info(EventCategory.SESSION, 
    `✅ [SwitchLanguage] Language switched: ${previousLanguage || 'none'} → ${normalizedCode} (${languageName})`);
  ...
  return {
    ...
    data: {
      ...
      message: `Language switched to ${languageName} (${normalizedCode}) with voice ${newVoice}`,
    },
    addToHistory: true,
  };
}
